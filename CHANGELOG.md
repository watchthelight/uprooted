...
## [v0.3.6-rc](https://github.com/watchthelight/uprooted/releases/tag/v0.3.6-rc) — 2026-02-18 (Pre-release)

### New
- **Message Logger plugin** — logs deleted and edited messages; deleted messages appear inline with red styling; flat-file persistence with configurable retention limit
- **Auto-updater** — polls GitHub releases and applies updates in-process; "Check for Updates" button in About settings; changes take effect on next Root restart
- **Developer update channel** — password-gated channel for pre-release builds
- **Custom ping highlight color** — override the mention/reply highlight color independently from the theme; persists across theme switches
- **Reddit link embeds** — dedicated embed handler with subreddit label and Reddit orange accent color
- **ClearURLs** — strips tracking parameters (utm_*, fbclid, gclid, etc.) from message URLs before sending
- **Silent Typing plugin** — hides the typing indicator so others can't see when you're composing

### Improvements
- Uprooted settings section repositioned above "App Settings" — no longer requires scrolling to reach
- Linux installer: 7-strategy Root auto-detection (glob patterns, .desktop files, running processes, PATH, locate, find)
- Plugin status: SentryBlocker and LinkEmbeds promoted Alpha → Beta
- Restart banners appear after plugin state changes or after an update is applied
- Diagnostics: "Open" button opens the log file directly in the file manager

### Fixed
- Theme flash when opening settings — multiple recolor passes on inject and tab switch
- Link embeds: all 4 image corners now round correctly
- Link embeds: Cloudflare/non-image responses rejected before display
- Link embeds: image URLs that serve HTML fall back to OpenGraph fetch
